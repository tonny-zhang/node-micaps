	    ...
	    var fn_return = function (points, items, is_peak){
	        ...
	        if(len >= 3){
	            ...
	            if(val_cache){
	                ...
	                ids.reverse();
	                var key_reverse = ids.join(',');
	                val_cache = arc_cache[key_reverse];
	                if(val_cache){
	                    val_cache.reverse();
	                    return val_cache;
	                }else{
	                    ...
	                    var result_new = [];
	                    if(result.length < 10){ //尽量保留小细节，减少计算
	                    	result_new = result; 
	                    }else{
		                    var prev_point = result.shift();
		                    result_new = [prev_point];
